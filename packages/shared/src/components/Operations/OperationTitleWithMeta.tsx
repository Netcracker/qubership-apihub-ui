...
export const OperationTitleWithMeta: FC<OperationTitleWithMetaProps> = memo<OperationTitleWithMetaProps>(
  ({ operation, link, onLinkClick, badgeText, openLinkInNewTab = false, onlyTitle = false }) => {
    ...
    return (
      <Box display="flex" flexDirection="column" width="100%">
        <Box display="flex" alignItems="center" gap={1} data-testid="OperationTitle">
          <OverflowTooltip title={title}>{titleNode}</OverflowTooltip>

          {badgeText && <CustomChip value={badgeText.toLowerCase()} label={badgeText} isExtraSmall />}
        </Box>
        {!onlyTitle && (
