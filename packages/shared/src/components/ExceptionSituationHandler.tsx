...
export const ExceptionSituationHandler: FC<ExceptionSituationHandlerProps> = memo<ExceptionSituationHandlerProps>(
  ({ homePath, showErrorNotification, redirectUrlFactory, children }) => {
    ...
    const packageRedirectFactory = useCallback(
      (newPackageId: string) => {
        ...
        if (redirectedPathname)
          return (
            redirectUrlFactory?.(location.pathname, searchParams, newPackageId) ??
    ...
    if (exceptionDetails.type === 404 || exceptionDetails.type === 500) {
      return <ErrorPage title={exceptionDetails.title} message={exceptionDetails.message} homePath={homePath} />
    }

    if (exceptionDetails.type === FETCH_REDIRECT_TYPE_PACKAGE) {
      return <PackageRedirectPage newId={exceptionDetails.id} href={packageRedirectFactory(exceptionDetails.id)} />
    }
