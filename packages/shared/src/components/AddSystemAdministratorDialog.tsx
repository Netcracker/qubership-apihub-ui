...
export const AddSystemAdministratorDialog: FC<AddSystemAdministratorDialogProps> = memo(
  ({ users, isUsersDataLoading, onConfirm, setUserSearch }) => {
    return (
      <PopupDelegate
...
export const AddSystemAdministratorPopup: FC<AddSystemAdministratorPopupProps> = memo<AddSystemAdministratorPopupProps>(
  ({ open, setOpen, users, isUsersDataLoading, setUserSearch, onConfirm }) => {
    const {
      handleSubmit,
      control,
      reset,
      formState: { errors },
    } = useForm<AddSystemAdministratorForm>()
    ...
    return (
      <DialogForm open={open} onClose={() => setOpen(false)} onSubmit={handleSubmit(onConfirmCallback)}>
