...
export const CommonLoginPageComponent: FC<CommonLoginPageComponentProps> = memo(
  ({ applicationName, onLogin, isLoading, isError }) => {
        ...
        <Grid container component="main" sx={{ height: '100vh', backgroundColor: 'white' }}>
          ...
          <Grid item component="div" xs={12} sm={8} md={5} sx={{ margin: 'auto' }}>
            ...
            >
              ...
              {isError && <Alert severity="error">Invalid credentials, please try again</Alert>}
              <Box component="form" sx={{ my: 2, width: 1 }} onSubmit={handleSubmit(onLogin)}>
                <Controller
                  name="username"
                  control={control}
                  render={({ field }) => (
                ...
                {ssoIntegration?.ssoIntegrationEnabled && (
                  <LoadingButton
                    variant="contained"
                    loading={isLoading}
