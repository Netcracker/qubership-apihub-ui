...
export const OperationChangesSubTable: FC<OperationChangesSubTableProps> = memo<OperationChangesSubTableProps>(
  ({ packageKind, changes, isLoading }) => {
      ...
      const result: ColumnDef<OperationChange>[] = [
        {
          ...
          cell: ({
            ...
            <Box
              ...
              height="32px"
              position="relative"
              data-testid="ChangeDescriptionCell"
            >
              <ChangeSeverityIndicator key={crypto.randomUUID()} severity={severity as ChangeSeverity} />
              <OverflowTooltip title={description}>
                <Typography noWrap ml={4} variant="inherit">
                  {description}
                </Typography>
              </OverflowTooltip>
            </Box>
    ...
    const data: OperationChange[] = useMemo(() => [...changes], [changes])
