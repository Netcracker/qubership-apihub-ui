...
async function updateBranchEditingMode(
  projectKey: string,
  branchName: string,
  branchEditingMode: boolean,
): Promise<void> {
  const projectId = encodeURIComponent(projectKey)
  const branch = encodeURIComponent(branchName)

  await editorRequestVoid(`/projects/${projectId}/branches/${branch}/editors`, {
    method: branchEditingMode ? 'POST' : 'DELETE',
  })
}

type UpdateBranchEditingMode = (mode: boolean) => void
