...
export const FileProblemsPanel: FC<ValidationProblemsPanelProps> = memo<ValidationProblemsPanelProps>(({ fileKey }) => {
  ...
  const columns: ReadonlyArray<Column<FileProblem>> = useMemo(() => {
    return [
        ...
        Cell: ({ value }) => (
          <>
            {value === ERROR_FILE_PROBLEM_TYPE && (
              <StatusMarker value={ERROR_STATUS_MARKER_VARIANT} title="Error" placement="left" />
            )}
            {value === WARN_FILE_PROBLEM_TYPE && (
              <StatusMarker value={WARNING_STATUS_MARKER_VARIANT} title="Warning" placement="left" />
            )}
            {value === INFO_FILE_PROBLEM_TYPE && (
              <StatusMarker value={DEFAULT_STATUS_MARKER_VARIANT} title="Info" placement="left" />
            )}
          </>
        ),
      },
      {
        ...
        Cell: ({
          row: {
            original: { externalFilePath, lineNumber, filePath },
          },
        }) => {
          if (externalFilePath) {
            return null
      ...
      },
      {
        ...
        Cell: ({
          value,
          row: {
            original: { externalFilePath, lineNumber },
          },
        }) => (
          <>
  ...
  return (
    ...
    >
      <ProblemsControls problems={problems} filter={filter} setFilter={setFilter} setSearchState={setSearchState} />
      <Placeholder
                ...
                <TableRow {...headerGroup.getHeaderGroupProps()}>
                  {headerGroups.map((headerGroup) =>
                    headerGroup.headers.map((column) => (
                      <TableCell {...column.getHeaderProps({ style: { width: column.width } })} key={column.id}>
...
const ProblemsControls: FC<ProblemsControlsProps> = memo<ProblemsControlsProps>(
  ({ problems, filter, setFilter, setSearchState }) => {
    ...
    return (
      <Box sx={{ display: 'flex', mb: 2, gap: 2, order: 1 }}>
        <CustomToggleButtonGroup value={filter} onClick={(value) => setFilter(value)}>
          <ToggleButton value={ERROR_FILE_PROBLEM_TYPE} sx={{ gap: 1 }}>
            <StatusMarker value={ERROR_STATUS_MARKER_VARIANT} title="Errors" placement="top" />
            <Typography fontSize={11}>{errorMessagesCount}</Typography>
          </ToggleButton>
          <ToggleButton value={WARN_FILE_PROBLEM_TYPE} sx={{ gap: 1 }}>
            <StatusMarker value={WARNING_STATUS_MARKER_VARIANT} title="Warnings" placement="top" />
            <Typography fontSize={11}>{warnMessagesCount}</Typography>
          </ToggleButton>
          <ToggleButton value={INFO_FILE_PROBLEM_TYPE} sx={{ gap: 1 }}>
            <StatusMarker value={DEFAULT_STATUS_MARKER_VARIANT} title="Infos" placement="top" />
            <Typography fontSize={11}>{infoMessagesCount}</Typography>
          </ToggleButton>
        </CustomToggleButtonGroup>

        <Box sx={{ flex: 1, maxWidth: 336 }}>
          <SearchBar placeholder="Search" onValueChange={setSearchState} />
        </Box>
      </Box>
    )
  },
