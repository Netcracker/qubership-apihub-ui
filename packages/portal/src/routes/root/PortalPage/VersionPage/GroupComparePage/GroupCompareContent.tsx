  ...
  const filteredGroupChanges: OperationChanges[] = useMemo(
    () =>
      groupChanges.filter(change => (selectedTag && change.metadata?.tags
        ? Array.isArray(change.metadata?.tags) && change.metadata?.tags.includes(selectedTag)
        ...
        >
          <Box pt={2}>
            {filteredGroupChanges.map((change) => {
              ...
              const metadata = metadataObject as
                & OperationChangesMetadata
                & Partial<RestChangesMetadata>
                & Partial<GraphQLChangesMetadata>
              const previousMetadata = previousMetadataObject as
                & OperationChangesMetadata
                & Partial<RestChangesMetadata>
                & Partial<GraphQLChangesMetadata>
                ...
                >
                  <Grid
                    ...
                    sx={{
                      ...
                      background: ACTION_TYPE_COLOR_MAP[operationAction as keyof typeof ACTION_TYPE_COLOR_MAP]
                        ?? '#F2F3F5',
                    }}
