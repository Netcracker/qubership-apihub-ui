...
function useData(
  ...
  const onSubmit = useMemo(() =>
    form.handleSubmit(({ originalGroup, changedGroup }) => {
      ...
      navigate({
        pathname: `/portal/packages/${defaultPackageKey}/${defaultVersionKey}/groups/${
          encodeURIComponent(changedGroup?.groupName ?? '')
        }/compare`,
        search: `${searchParams}`,
      })
  ...
  const originalGroupOptions = useMemo(() => {
    return originalOperationGroups.filter(({ groupName }) =>
  ...
  const changedGroupOptions = useMemo(() => {
    return changedOperationGroups.filter(({ groupName }) =>
