...
export const OperationGroupTable: FC<OperationGroupTableProps> = memo<OperationGroupTableProps>(
  ({ groups, isLoading, isPackage, onEditContent, onEdit, onDelete, onPublish }) => {
    ...
    const columns: ColumnDef<OperationGroup>[] = useMemo(() => {
      const result: ColumnDef<OperationGroup>[] = [
        {
          ...
          cell: ({
            row: {
              original: { groupName, exportTemplateFileName },
            },
          }) => (
            <Box display="flex" alignItems="center">
              <TextWithOverflowTooltip tooltipText={groupName}>{groupName}</TextWithOverflowTooltip>
        ...
        defineDefaultColumn(DESCRIPTION_COLUMN_ID, 'Description', (group) => group.description),
        defineDefaultColumn(API_TYPE_COLUMN_ID, 'API Type', ({ apiType }) =>
          apiType ? API_TYPE_TITLE_MAP[apiType]! : '',
        ),
        defineDefaultColumn(OPERATIONS_NUMBER_COLUMN_ID, 'Number of Operations', (group) =>
          String(group.operationsCount),
        ),
        {
      ...
      if (isPackage) {
        insertIntoArrayByIndex(
          result,
          defineDefaultColumn(GROUP_TYPE_COLUMN_ID, 'Type', (group) =>
            group.isPrefixGroup ? GROUP_TYPE_REST_PATH_PREFIX : GROUP_TYPE_MANUAL,
          ),
          1,
        )
      }
...
function defineDefaultColumn(
  ...
  return {
    ...
    cell: ({ row }) => {
      ...
      return <TextWithOverflowTooltip tooltipText={value}>{value}</TextWithOverflowTooltip>
    },
  }
}
