...
export const DeprecatedItemsSubTable: FC<SubTableProps> = memo<SubTableProps>(
  ({
    value: {
      getVisibleCells,
      original: {
        operation: { operationKey, packageRef },
      },
    },
  }) => {
      ...
      () => [
        {
          ...
          cell: ({
            ...
            <Box sx={{ ml: 4 }} display="flex" alignItems="center" height="32px" position="relative">
              <TextWithOverflowTooltip tooltipText={description}>{description}</TextWithOverflowTooltip>
            </Box>
          ),
        },
        {
          ...
          cell: ({
            row: {
              original: { deprecatedInPreviousVersions },
            },
          }) => {
    ...
    const data: DeprecatedItem[] = useMemo(() => [...deprecatedItems], [deprecatedItems])
