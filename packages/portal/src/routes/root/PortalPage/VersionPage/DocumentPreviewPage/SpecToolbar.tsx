...
type SpecViewMode = typeof DOC_SPEC_VIEW_MODE | typeof RAW_SPEC_VIEW_MODE
...
export const SpecToolbar: FC = memo(() => {
  ...
  return (
    <Toolbar
      breadcrumbs={<PackageBreadcrumbs packageObject={packageObject} versionKey={versionId} showPackagePath={true} />}
      header={
          ...
          <>
            {specViewMode === DOC_SPEC_VIEW_MODE && (
              <>
                {DOC_VIEW_COMPATIBLE_TYPES.includes(type ?? '') && (
                  <Toggler modes={SCHEMA_VIEW_MODES} mode={schemaViewMode} onChange={setSchemaViewMode} />
                )}
              </>
            )}
            <SpecViewToggler
              mode={specViewMode as SpecViewMode}
              modes={[DOC_SPEC_VIEW_MODE, RAW_SPEC_VIEW_MODE]}
            ...
            >
              {isOpenApiSpecType(type) ? (
                ...
                <MenuItem onClick={() => downloadPublishedFileDoc()}>Download</MenuItem>
              )}
            </MenuButton>
          </>
        )
