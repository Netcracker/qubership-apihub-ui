...
export const ReferenceRow: FC<ReferenceRowProps> = memo<ReferenceRowProps>(
  ({
    ...
    pack: { key, version, name, kind, status, deletedAt, latestRevision },
    versionReferences,
    level,
    onRemove,
    added,
    ...
    useEffect(() => {
      ...
      if (
        !addedVersionReferences.references?.some(
          ({ packageRef }) => addedVersionReferences.packages![packageRef!].key === packageId,
        )
      ) {
        return
      }
    ...
    const updateCollapseKeys = useCallback(
      (key: Key) => {
        ...
        setCollapsedKeys((previousKeys) =>
          !previousKeys.includes(key) ? [...previousKeys, key] : previousKeys.filter((id) => id !== key),
        )
      },
      ...
      <>
        <TableRow hover tabIndex={-1} key={key} sx={{ p: 0 }}>
          ...
          <TableCell key="version" data-testid="VersionCell">
            <TextWithOverflowTooltip tooltipText={versionKey}>{versionKey}</TextWithOverflowTooltip>
          </TableCell>
