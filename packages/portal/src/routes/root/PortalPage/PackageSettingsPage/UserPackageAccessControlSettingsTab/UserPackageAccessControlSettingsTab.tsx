...
import { UserAccessControlTable } from '@apihub/routes/root/PortalPage/PackageSettingsPage/UserPackageAccessControlSettingsTab/UserAccessControlTable'
...
import { UserRolesDialog } from '@apihub/routes/root/PortalPage/PackageSettingsPage/UserPackageAccessControlSettingsTab/UserRolesDialog'
...
export const UserPackageAccessControlSettingsTab: FC<PackageSettingsTabProps> = memo<PackageSettingsTabProps>(
  ({ packageObject }) => {
    ...
    const [availablePackageRoles, isRolesPackageLoading] = useAvailablePackageRoles(
      packageObject.key,
    ...
    const isLoading = useMemo(
      () => isRolesPackageLoading && isPackageMembersLoading,
      [isRolesPackageLoading, isPackageMembersLoading],
    )
    ...
    const handleAddPackageMemberRole = useCallback(
    ...
    return (
      <Box height="100%">
          ...
          action={
            <Box sx={{ display: 'flex', mb: 2, gap: 2, order: 1 }}>
              ...
              <Box sx={{ flex: 1, maxWidth: 240 }}>
                <SearchBar placeholder="Search" onValueChange={onChangeSearchValue} data-testid="SearchUser" />
              </Box>
            </Box>
          }
          body={
        ...
        />
        <UserRolesDialog permissions={permissions} roles={roles} isLoading={isRolesLoading || isPermissionsLoading} />
      </Box>
    )
  },
)
