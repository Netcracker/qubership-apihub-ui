...
export const UserAccessControlTable: FC<UserAccessControlTableProps> = (props) => {
  const { packageKey, roles, availablePackageRoles, packageMembers, hasUserAccessManagementPermission, searchValue } =
    props
  ...
  const onDeletePackageMember = useCallback(
    (packageKey: string, userId: string, userName: string) => {
  ...
  const onChangePackageMemberRole = useCallback(
    (userId: Key, roleId: Key, checked: boolean) => {
  ...
  const horizontalItemRender = useCallback((role: Role) => {
    return (
      <TableCell key={role.key} sx={{ textAlign: 'center', width: '121px' }}>
  ...
  const verticalItemRender = useCallback((packageMember: PackageMember) => {
    return (
      <TableCell key={packageMember.user.id} className="leftSticky" data-testid="Cell-user">
        <UserView name={packageMember.user.name} avatarUrl={packageMember.user.avatarUrl} />
      </TableCell>
    )
  }, [])

  const matrixItemRender = useCallback(
    (key: string, item: MatrixItem) => {
      return (
