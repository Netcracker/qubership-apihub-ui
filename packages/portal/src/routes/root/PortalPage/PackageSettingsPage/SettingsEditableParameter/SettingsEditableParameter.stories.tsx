...
export const ChipsWithContentOverflow: Story = {
  ...
  args: {
    ...
    children: (
      <>
        {Array.from({ length: 30 }, (_, i) => <Chip key={i} label={`tag-${i + 1}`} size="small" />)}
      </>
    ),
  },
}
...
export const TypographyWithLongText: Story = {
  ...
  args: {
    ...
    onEdit: () => alert('Edit button clicked'),
    children: (
      <OverflowTooltip title="This is a very long text that will overflow the container and trigger the ellipsis and tooltip behavior when hovered. The text is deliberately long to demonstrate how overflow works with Typography component.">
        <Typography variant="body2" textOverflow="ellipsis" overflow="hidden" noWrap>
