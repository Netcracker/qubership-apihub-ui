  ...
  return (
    <>
      ...
      <IconButton data-testid="AppUserAvatar" size="large" color="inherit">
        {' '}
        { authorization?.user.avatarUrl ? <Avatar src={authorization.user.avatarUrl} /> : <UserAvatar size="medium" name={authorization?.user.name ?? ''} /> }
        {' '}
      </IconButton>
...
function useMergedGraphQlSpec(options: {
  ...
  return useMemo(() => {
    ...
    try { const mergedTypeDefs = mergeTypeDefs(specsRaw); return print(mergedTypeDefs) } catch (e) { console.error(e); return null }
  }, [enabled, specsRaw],
  )
}
